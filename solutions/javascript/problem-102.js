...
module.exports.run = () => {
    ...
    const getSides = (p1, p2, p3) => {
        return [
            getDistanceBetweenPoints(p1, p2),
            getDistanceBetweenPoints(p2, p3),
            getDistanceBetweenPoints(p3, p1)
        ];
    };
    ...
    let currentTriangleCoOrdinates,
        p1,
        p2,
        p3,
        a1,
    ...
    for (let i = 0; i < inputTriangles.length; i++) {
        currentTriangleCoOrdinates = inputTriangles[i].split(',');
        p1 = {
            x: currentTriangleCoOrdinates[0],
            y: currentTriangleCoOrdinates[1]
        };
        p2 = {
            x: currentTriangleCoOrdinates[2],
            y: currentTriangleCoOrdinates[3]
        };
        p3 = {
            x: currentTriangleCoOrdinates[4],
            y: currentTriangleCoOrdinates[5]
        };

        a1 = getAreaOfTriangle.apply(this, getSides(p, p2, p3));
        a2 = getAreaOfTriangle.apply(this, getSides(p1, p, p3));
        a3 = getAreaOfTriangle.apply(this, getSides(p1, p2, p));
        a = getAreaOfTriangle.apply(this, getSides(p1, p2, p3));

        if (Math.abs((a1 + a2 + a3) - a) < 1) {
            count++;
        }
    }
